...
<button
  type="button"
  class="bg-gray-200 dark:bg-gray-700 relative inline-flex flex-shrink-0 h-6 w-11 border-2 border-transparent rounded-full cursor-pointer transition-colors ease-in-out duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
  role="switch"
  aria-checked="false"
...
>
  ...
  <span
    class="{enabled
      ? classNames.wrapperEnabled
      : classNames.wrapperNotEnabled} pointer-events-none relative inline-block h-5 w-5 rounded-full bg-white shadow transform ring-0 transition ease-in-out duration-200"
  >
    <span
      class="{enabled
        ? classNames.firstOptionEnabled
    ...
    <span
      class="{enabled
        ? classNames.secondOptionEnabled
